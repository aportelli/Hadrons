...
void ProjAccumRunner(std::vector<Field> &in, std::vector<Field> &out, unsigned int eb, unsigned int sb)
{
    ...
    for (int i = 0; i < evSize; i += eb) // need to chnge loop structure.
    {
        ...
        eigenVec.resize(evBlockSize,g);
        eigenval.resize(evBlockSize);
        ...
        //for (unsigned int e = 0; e < eigenval.size(); e++)
        for (auto &e: eigenval)
        {
            //eigenval[e] = 1.; // replace with random pick if grid rng supports not vector structures
            random(RNG,e);
             ...
             unsigned int minBatchSizeS, unsigned int maxBatchSizeS)
{
    ...
    for (int eb = minBatchSizeE; eb <= maxBatchSizeE; eb++)
    {
        for (int sb = minBatchSizeS; sb <= maxBatchSizeS; sb++)
        {
            ...
            std::cout << GridLogMessage << "Iteration: " << count << std::endl;
            ...
            // print some average data

            std::cout << GridLogMessage << "Result, average: " << w1.Elapsed()/iter << std::endl;         
